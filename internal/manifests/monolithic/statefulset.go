...
func configureGateway(opts Options, sts *appsv1.StatefulSet) error {
	...
	if opts.CtrlConfig.Gates.OpenShift.ServingCertsService {
		args = append(args, []string{
			...
			fmt.Sprintf("--tls.healthchecks.server-name=%s", naming.ServiceFqdn(tempo.Namespace, tempo.Name, "")),
	...
	if tempo.Spec.Multitenancy.TenantsSpec.Mode == v1alpha1.ModeOpenShift {
		sts.Spec.Template.Spec.Containers = append(sts.Spec.Template.Spec.Containers, gateway.NewOpaContainer(opts.CtrlConfig, tempo.Spec.Multitenancy.TenantsSpec, opaPackage))
	}
	...
	if opts.CtrlConfig.Gates.OpenShift.ServingCertsService {
		...
		err = manifestutils.MountCertSecret(&sts.Spec.Template.Spec, containerName, naming.ServingCertName(manifestutils.TempoMonolithComponentName, tempo.Name), servingCertDir)
		if err != nil {
			return err
		}
	}
