...
// CreateOrRotateCertificates handles the TempoMonolithic client and serving certificate creation and rotation
// including the signing CA and a ca bundle or else returns an error. It returns a hash of all generated
// certificates (to trigger pod restarts on rotation) and a degrade-condition-worthy error if building
// the manifests fails for any reason.
func CreateOrRotateCertificates(ctx context.Context, log logr.Logger,
	req ctrl.Request, k client.Client, s *runtime.Scheme, fg configv1alpha1.FeatureGates, cs map[string]string) (string, error) {
	...
	return certrotation.CertsHash(objects), nil
}
